...
import { mobileOfflineStore } from "@/lib/offline-store";
...
export const mobileOfflineQueue = createOfflineQueueBridge(mobileOfflineStore, () =>
  useAuthStore.getState().user?.id ?? null,
);

export async function flushMobileOfflineQueue(
  queryClient: import("@tanstack/react-query").QueryClient,
) {
  const userId = useAuthStore.getState().user?.id;
  if (!userId) return { synced: 0, remaining: 0, failed: false };
  const result = await flushOfflineQueue(mobileOfflineStore, userId, api, queryClient);
  notifyPending();
  return result;
}
