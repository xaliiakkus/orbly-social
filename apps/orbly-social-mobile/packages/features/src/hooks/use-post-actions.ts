  ...
  const queueLike = useCallback(
    async (next: boolean) => {
      ...
      await offlineQueue.enqueue({
        type: next ? "posts.like" : "posts.unlike",
        postId,
      });
      applyPostLikeToCache(qc, postId, next);
      return true;
    },
    [offlineQueue, postId, qc],
  );
  ...
  const toggle = useCallback(async () => {
    ...
    try {
      if (offlineQueue?.isOffline()) {
        await queueLike(next);
        return;
      }
      await mutation.mutateAsync(next);
    } catch (e) {
      if (await queueLike(next).catch(() => false)) return;
      if (isQueueableError(e)) return;
      setLiked(!next);
      setCount((c) => Math.max(0, c + (next ? -1 : 1)));
    } finally {
      busyRef.current = false;
    }
  }, [liked, mutation, offlineQueue, queueLike, postId]);
